...
**Experience הוא טאב נפרד, לא שדה בתוך "עיצוב"** (§52 במסמך הסופי):
המשתמש חייב לדעת ברור מתי הוא נכנס לסביבת חוויית הגלילה. ברמת הדף:
...
מעבר בין המצבים **אף פעם לא מוחק דאטה** (§53/§95): הפעלת Experience
יוצרת מצב פתיחה נקי (scene ריק ראשון); כיבוי Experience משבית את
ה-runtime אך **משמר** את ה-JSON — הפעלה מחדש משחזרת בדיוק את מה
שהיה.
...
לוודא מול הקוד בפועל, וכל `PARTIAL`/`FAIL` מקבל תוכנית תיקון.
